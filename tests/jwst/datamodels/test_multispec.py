...
from stdatamodels.jwst.datamodels import WFSSCombinedSpecModel, WFSSMultiSpecModel, WFSSSpecModel
...
def test_legacy_wfss_combinedspec(tmp_path):
    ...
    # Create a np recarray with old-style columns
    default_spectable = WFSSCombinedSpecModel().get_default("spec_table")

    # make a WFSSCombinedSpecModel with a few extensions
    fname = tmp_path / "wfss_combinedspec.fits"
    with WFSSCombinedSpecModel() as model:
        model.spec_table = default_spectable
        model.save(fname)
    ...
    with fits.open(fname) as hdulist:
        ...
        # Now try to instantiate a WFSSCombinedSpecModel from this hdulist
        # and check that the columns have been restored
        with WFSSCombinedSpecModel(hdulist) as model:
